...
def  main():
  ...
  installer = "\installer\VinoEcho Installer.exe"
   
  if demo:
    installer = "\installer\VinoEcho Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\VinoEcho manual.pdf" 
  ]
  ...
  ZIPNAME = "VinoEcho-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "VinoEcho-v" + FULLVERSIONSTR + "-win-demo.zip"
